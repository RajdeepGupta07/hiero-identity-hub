...
// Public (unauthenticated) verification — used by shareable /verify/:hash page
export const publicVerifyCredential = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (error || !cred) {
      return { valid: false, result: "invalid", reason: "Credential not found" };
    }
